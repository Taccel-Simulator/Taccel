...
def val(y, sim: 'ASRModel', scale: float, reduce_each: bool):
    ...
    wp.launch(kernel=compute_rigidity_neohookean_energy_val, dim=sim.affine_body_num, inputs=[sim.sim_cache.affine_body_energy, y, sim.vol_body, sim.E_body, sim.nu_body, wp.float64(scale), helper.affine_has_constraint])
    if reduce_each:
...
def grad(y, sim: 'ASRModel', scale, gradient):
    helper = sim.kinematic_helper
    wp.launch(kernel=compute_rigidity_neohookean_energy_grad, dim=sim.affine_body_num, inputs=[gradient, y, sim.vol_body, sim.E_body, sim.nu_body, wp.float64(scale), helper.affine_has_constraint, sim.body_env_id, sim.env_states])
...
def hess(y, sim: 'ASRModel', scale, project_pd):
    helper = sim.kinematic_helper
    wp.launch(kernel=compute_rigidity_neohookean_energy_hess, dim=sim.affine_body_num, inputs=[sim.hess_affine_diag, y, sim.vol_body, sim.E_body, sim.nu_body, wp.float64(scale), wp.bool(project_pd), helper.affine_has_constraint, sim.body_env_id, sim.env_states])