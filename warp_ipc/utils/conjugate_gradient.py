...
@wp.kernel
def axpy(y: wp.array(dtype=wp.vec3d), x: wp.array(dtype=wp.vec3d), env_beta: wp.array(dtype=wp.float64), ret: wp.array(dtype=wp.vec3d), body_env_id: wp.array(dtype=wp.int32), node2env: wp.array(dtype=wp.int32), n_affine_dofs: wp.int32, affine_verts_num: wp.int32):
    pass

@wp.kernel
def cg_one_iter(x: wp.array(dtype=wp.vec3d), r: wp.array(dtype=wp.vec3d), q: wp.array(dtype=wp.vec3d), env_alpha: wp.array(dtype=wp.float64), p: wp.array(dtype=wp.vec3d), Ap: wp.array(dtype=wp.vec3d), diag_inv: wp.array(dtype=wp.mat33d), body_env_id: wp.array(dtype=wp.int32), node2env: wp.array(dtype=wp.int32), n_affine_dofs: wp.int32, affine_verts_num: wp.int32):
    pass
...
def conjugate_gradient(x: wp.array, A: BsrMatrix, b: wp.array, sim: 'ASRModel', tol=0.001, max_iters: None | int=500):
    ...
    def _array_inner_per_env(a: wp.array, b: wp.array, out: wp.array):
        ...
        if sim.affine_body_num > 0:
            wp.to_torch(out).scatter_add_(0, wp.to_torch(sim.body_env_id[:sim.affine_body_num]).to(torch.int64).repeat_interleave(4), wp.to_torch(sim.sim_cache.cg_inner_tmp[:sim.n_affine_dofs // 3]))
    ...
    for iter in range(max_iters):
        ...
        wp.launch(kernel=cg_one_iter, dim=num_block, inputs=[x, sim.sim_cache.cg_r, sim.sim_cache.cg_q, sim.sim_cache.cg_alpha, sim.sim_cache.cg_p, sim.sim_cache.cg_Ap, sim.sim_cache.cg_diag_inv, sim.body_env_id, sim.node2env, sim.n_affine_dofs, sim.affine_verts_num], device=sim.device)
        ...
        wp.launch(kernel=axpy, dim=num_block, inputs=[sim.sim_cache.cg_q, sim.sim_cache.cg_p, sim.sim_cache.cg_beta, sim.sim_cache.cg_p, sim.body_env_id, sim.node2env, sim.n_affine_dofs, sim.affine_verts_num], device=sim.device)
